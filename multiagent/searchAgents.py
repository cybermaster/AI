...
# then recursevly the mahatten distance to the closest corner from
...
##    passes - 1 point 8000 + nodes
#    foodGridCP = foodGrid.copy()
#    close2 = distClose(position, foodGridCP)
#    foodGridCP[close2[1][0]][close2[1][1]] = False
#    far = distFar(close2[1], foodGridCP)
##
#    value = (close2[0]  + far[0])
#    
#    if value2 > value1 :
#        print value2
...
class ApproximateSearchAgent(Agent):
    ...
    def getAction(self, state):
        ...
        def makeActions():
            ...
            # finds maze distance between two points, returns distance
            def distanceU(p1, p2, state) :
                if heuristicInfo.has_key( (p1,p2) ) :
                    return problem.heuristicInfo[(p1,p2)]
                else :
                    walls = self.walls
                    fringe = util.PriorityQueue()
                    ...
                    while True : 
                        ...
                        #print nextNode
                        if nextNode[0] == p2 :
                            heuristicInfo[(p1,p2)] = nextNode[1]
                            return nextNode[1]
                        if not nextNode[0] in explored :
                            explored.add(nextNode[0])
                            nx, ny = nextNode[0]
                            ...
                            for s in successors :
                                if walls[s[0]][s[1]] == False :
                                    ...
                                    fringe.push( (s, path), path )
            # finds the closest food by maze distance to position                    
            def distCloseU(position, foodGrid, state) :
                gridInfo = foodGrid.packBits()
                ...
                for i in range(gridInfo[0]) :
                    for j in range(gridInfo[1]) :
                        if foodGrid[i][j] == True :
                            dist = (distanceU(position, (i,j), state), (i,j))
                            #dist = ( ( abs(position[0] - i) + abs(position[1] - j) ), (i,j) )
                            if value == None :
                                value = dist
                            if dist[0] < value[0] :
            ...
            # returns the value of min spanning tree, created from maze distances between food
            def minSpan(position, foodGrid, state) :
                ...
                for i in range(len(foods)) :
                    ...
                    V.add(food)
                    for j in range(len(foods)) :
                        dist = distanceU(food, foods[j], state)
                        # go back to... for reg
                        #dist = util.manhattanDistance(food, foods[j])
                        queue.push( (food, foods[j], dist), dist)
            ...
            def foodSolve(position, foodGrid):
                foodGridCP = foodGrid.copy()
                close = distCloseU(position, foodGridCP, state)
                min = minSpan(position, foodGridCP, state)
                value = min + close[0]
    #        print gridCopy(foodGrid)
    #    #   works < 300 nodes ...      
    #        foodGridCP = foodGrid.copy()
    #        close = distCloseU(position, foodGridCP, state, problem)
    #        min = minSpan(position, foodGridCP, state, problem)
    ...
    #        self.actions  = self.searchFunction(problem) # Find a path
           ...
           #BILL HERE
           # want to make a food search problem here
           # just make one with all the food, even though it wont be able to solve, then Ill fix it
            
            
            foodGrid = state.getFood()
            position = self.start[0]
            
            def aStarS(problem, heuristic, foodGrid):
            ...
            print 'foodGrid'
            print state.data.food
            problem = FoodSearchProblem(state)
            print state
            path = aStarS(problem, foodHeuristic)
        ...
        grids = gridCopy(state.getFood())
        state.data.food = grids[0]
        print state.data.food
